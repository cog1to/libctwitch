...
/**
 * Tries to obtain app access token given client ID and secret, and a list of
 * scopes.
 *
 ...
 *
 * @return Either a valid twitch_helix_auth_token struct filled with token data,
 * or NULL in case something went wrong. It's caller's responsibility to
 * deallocate the token with twitch_helix_auth_token_free().
 */
