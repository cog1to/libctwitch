...
/**
 * Returns a pointer to twitch_v5_user structure describing user found by given
 * user ID.
 *
 * @param id User ID.
 * @param client_id Twitch API client ID.
 * @return Dynamically allocated twitch_v5_user struct describing user. You have
 * to manually free the memory using twitch_v5_user_free() function.
 */
 ...
 * to manually free the memory using twitch_v5_user_free() function.
 */
twitch_v5_user *twitch_v5_get_user_by_username(
 ...
 *
 * @param client_id Twitch API client ID.
 * @param usernames_count Size of the list of usernames. Should not exceed 100,
 * accodring to API docs.
 * @param usernames List of usernames/logins to check.
 *
 * @return List of users. */
twitch_v5_user_list *twitch_v5_get_users(
	const char *client_id,
	int usernames_count,
 ...
 *
 * @return Follow data if user does follow the channel, or NULL otherwise. Don't
 * forget to free the twitch_v5_follow struct with twitch_v5_follow_free()
 * function.
 */
twitch_v5_follow *twitch_v5_check_user_follow(
	const char *client_id,
	const char *user_id,
 ...
 * @param sortby Sort property. Can be either "created_at", "last_broadcast", or
 * "login".
 * @param limit Page size.
 * @param offset List offset.
 * @param total Used to return the total number of items in the list reported by
 * Twitch API.
 *
 * @return List of follows. Deallocate with twitch_v5_follow_list_free().
 */
twitch_v5_follow_list *twitch_v5_get_user_follows(
	const char *client_id,
	const char *user_id,
 ...
 */
twitch_v5_follow_list *twitch_v5_get_all_user_follows(
	const char *client_id,
	const char *user_id,
	const char *direction,
	const char *sortby
