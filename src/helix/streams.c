...
string_t *helix_streams_url_builder(void *params, int limit, const char *after) {
  char buffer[128];
  ...
  if (sparams->language != NULL) {
    string_append_format(url, "%slanguage=%s", is_first_param ? "?" : "&", sparams->language);
    is_first_param = false;
  }

  for (int idx = 0; idx < sparams->logins_count && idx < MAX_USERS_COUNT; idx++) {
    string_append_format(url, "%suser_login=%s", is_first_param ? "?" : "&", sparams->logins[idx]);
    is_first_param = false;
  }

  for (int idx = 0; idx < sparams->users_count && idx < MAX_USERS_COUNT; idx++) {
    string_append_format(url, "%suser_id=%d", is_first_param ? "?" : "&", sparams->users[idx]);
    is_first_param = false;
  }
